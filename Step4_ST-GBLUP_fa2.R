...
GEBVs_ST_GBLUP_fa2 <- data.frame() |>
  bind_rows(GEBVs_mod2_YLD |> mutate(trait='grain_yield'),
            GEBVs_mod2_TW |> mutate(trait='test_weight'),
            GEBVs_mod2_HD |> mutate(trait='heading_time'),
            GEBVs_mod2_HT |> mutate(trait='plant_height'),
            GEBVs_mod2_MAT |> mutate(trait='maturity')) |>
  glimpse()

write.csv(GEBVs_ST_GBLUP_fa2,'Data/ST-GBLUP_fa2_gebvs_long.csv')

write.csv(
  GEBVs_ST_GBLUP_fa2 |>
    select(-status) |>
    pivot_wider(names_from = trait, values_from = solution) |>
    arrange(desc(grain_yield )) |>
    glimpse(),
  'Data/ST-GBLUP_fa2_gebvs_wide.csv')

save.image('Data/ST-GBLUP_fa2.RData')