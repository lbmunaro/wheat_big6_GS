...
mod1_gy <- asreml(fixed=predicted.value~1,
                  random=~fa(study,2):vm(germplasm, Ginv.sparse)+study,
                  weights=weight,
                  family=asr_gaussian(dispersion = 1),
                  data=blues_filter |> filter(trait == 'grain_yield_bu_ac'),
                  na.action = na.method(y='include', x='include'),
...
# GEBVs
GEBVs_mod1_gy <- predict(mod1_gy, classify='germplasm', pworkspace='2gb')$pval

hist(GEBVs_mod1_gy$predicted.value)
